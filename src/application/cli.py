...
def main():
    parser = argparse.ArgumentParser(
        description="Process input files using ReadManager, SplitManager, and ChunkManager to produce chunks."
    )
    parser.add_argument(
        "config_file",
        nargs="?",
    ...
    for file in tqdm(files, desc="Processing files", unit="file"):
        ...
        try:
            # read_file expects just the filename relative to the input directory.
        ...
        saved_files = chunk_manager.save_chunks(chunks, base_filename, original_extension, splitter_method)
